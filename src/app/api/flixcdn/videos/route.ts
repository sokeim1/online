...
import { listVideoseedCatalogFromDb } from "@/lib/videoseedIndex";
...
function ratingValue(v: { kp_rating?: unknown; imdb_rating?: unknown }): number {
  const a = parseRating(v.kp_rating);
  const b = parseRating(v.imdb_rating);
...
export async function GET(req: Request) {
  ...
  const cacheKey = `videos:${offset}:${safeLimit}:type=${type ?? ""}:year=${safeYear ?? ""}:genre=${genres.join(",")}:country=${country ?? ""}:sort=${sortByRating ? "rating" : ""}`;
  ...
  if (!forceUpstream && hasDatabaseUrl()) {
    try {
      const r = await listVideoseedCatalogFromDb({
        offset,
        limit: safeLimit,
        type,
        year: safeYear,
  ...
  try {
    const requestOpts = debug ? { timeoutMs: 10000, attempts: 2 } : { timeoutMs: 6000, attempts: 2 };
    ...
    const outRaw = results
      .flatMap(({ kind, r }) =>
        (r.data ?? []).map((x) => {
          ...
          const y = parseVideoseedYear((x as any).year);
    ...
    let filteredAll = outRaw.filter((x) => {
    ...
    if (sortByRating) {
      filteredAll = await enrichRatings(filteredAll);
    ...
    const total = results.length === 1 ? results[0]!.r.total ?? filteredAll.length : null;
    const lastPage = total != null && total > 0 ? Math.max(1, Math.ceil(total / safeLimit)) : anyNext ? safePage + 1 : safePage;
    ...
    const res = NextResponse.json({
      ...
      meta: {
        current_page: safePage,
        from: filteredAll.length ? offset + 1 : null,
        last_page: lastPage,
        links: [],
        path: "",
        per_page: safeLimit,
        to: filteredAll.length ? offset + filteredAll.length : null,
        total: total ?? filteredAll.length,
      },
      success: true,
      message: "",
      source: "videoseed",
